...
description: Exploring the new way to sort arrays in JavaScript with `toSorted`
date: 2023-11-30
---
...
console.log(
  `Time to sort ${arr.length} nanoid strings with [...arr].sort(): ${
    end - start
  } ms`,
);
...
console.log(
  `Time to sort ${arr.length} nanoid strings with arr.toSorted(): ${
    endBeta - startBeta
  } ms`,
);
```
